...
class CreateProfileUpdateFromIssueTests(unittest.TestCase):
    ...
    def test_main_converts_webp_photo_only_update_and_outputs_add_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo_root = pathlib.Path(tmpdir)
            profiles_root = repo_root / "_profiles"
            ...
            buffer = io.BytesIO()
            Image.new("RGBA", (16, 16), color=(12, 34, 56, 128)).save(buffer, format="WEBP", lossless=True)
            ...
            with (
                ...
                mock.patch.object(
                    MODULE.blog_post_automation,
                    "download_attachment",
                    return_value=(buffer.getvalue(), "image/webp"),
                ),
            ...
            with Image.open(images_root / "naoto_inui.png") as photo:
                self.assertEqual("PNG", photo.format)
                self.assertEqual((16, 16), photo.size)
