document.addEventListener('DOMContentLoaded', () => {
  ...
  const simulation = d3
    .forceSimulation()
    .force('link', d3.forceLink().id((d) => d.id).distance((link) => 90 - Math.min(link.weight, 6) * 6))
    .force('charge', d3.forceManyBody().strength((node) => (node.id === focusId ? -480 : -240)))
    .force('center', d3.forceCenter(width / 2, height / 2))
    .force('collision', d3.forceCollide().radius((node) => nodeRadius(node) + 8));
  ...
  function centerFocusNode() {
    const node = nodeLayer.selectAll('circle').data().find((entry) => entry.id === focusId);
    if (!node) {
      return;
    }
    centerNode(node);
