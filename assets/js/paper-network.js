document.addEventListener("DOMContentLoaded", () => {
  ...
  whenVisible(graphElement, async () => {
    let network;
    try {
      network = await loadJsonData(dataElement);
    } catch (error) {
      if (detailsElement) {
        renderMessage(detailsElement, i18n.unavailable);
      }
      graphElement.textContent = i18n.unavailable;
      return;
    }
